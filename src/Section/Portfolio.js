...
import { Container, Tab, Box, Fade, Slide, ButtonBase } from "@mui/material";
...
const Portfolio = () => {
    const [Items, setItems] = useState(() => Portfolios.filter(curElem => curElem.filter === "web"));
    ...
    return (
        <Container maxWidth={false} disableGutters as="section" sx={styles.Container} id="portfolio">
            ...
            <Box sx={{ position: "relative" }}>
                <Box sx={styles.ScrollWrapper} id="portfolio-scroll-container">
                    <Work works={Items} />
                </Box>
                <ButtonBase 
                    onClick={() => document.getElementById('portfolio-scroll-container').scrollBy({left: -400, behavior: 'smooth'})}
                    sx={{...styles.ScrollButton, left: "-20px"}}
                >
                    <ArrowBackIosNewIcon />
                </ButtonBase>
                <ButtonBase 
                    onClick={() => document.getElementById('portfolio-scroll-container').scrollBy({left: 400, behavior: 'smooth'})}
                    sx={{...styles.ScrollButton, right: "-20px"}}
                >
                    <ArrowForwardIosIcon />
                </ButtonBase>
            </Box>
